...
import { deleteFuelEntry } from "../../services/fuel/fuel.service";
...
export const execute = withErrorHandling(
  requireRole("owner")(async (event, auth): Promise<APIGatewayProxyResult> => {
    ...
    const entryId = event.pathParameters?.entryId as string;

    return sendSuccessResponse(
      await deleteFuelEntry(requester, vehicleId, entryId),
    );
  }),
);
